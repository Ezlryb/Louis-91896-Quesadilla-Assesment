...
def get_int(question, lower, upper):
    """Input question that will be repeated in between clears (clears all other text.) Return the users input if it is an integer between upper and lower."""
    os.system('clear')
    while True:
        num = input(question + "\n> ")
        os.system('clear')
...
def get_expected_input(question, possible_inputs):
    """Input question and a lsit of valid inputs and loops until the users answer is one of the possible inputs then returns that as a string."""
    while True:
...
def year():
    """Return card's expiration year as int."""
    return get_int("What year does your card expire?", 1950, 2050)


def month():
    """Return card's expiration month in 1, 2 ect. Format as int."""
    while True:
        message = "Eg. ("
...
def security_code():
    """Get the user to input 3 digit number."""
    os.system('clear')
    while True:
...
def checkout():
    """Print checkout menu and get the users information."""
    boolean = True
    while boolean:
...
def cart():
    """Return ui for cart as well as all your items."""
    message = "\n\n    **** Cart ****\n\n"
    total = 0
...
def main():
    """Run while loop with all the user's options."""
    global order
    while True:
